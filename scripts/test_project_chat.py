...
def test_project_chat():
    ...
    response = client.post(
        ...
        data=json.dumps({'session_id': session_id, 'action': 'start', 'message': 'My AI Art Gallery'}),
    ...
    response = client.post(
        '/api/v1/project/chat/stream/',
        data=json.dumps({'session_id': session_id, 'action': 'submit', 'message': '2'}),  # Image collection
        content_type='application/json',
    )
    ...
    response = client.post(
        '/api/v1/project/chat/stream/',
        data=json.dumps({'session_id': session_id, 'action': 'submit', 'message': 'yes'}),
        content_type='application/json',
    )
    ...
    if project_id:
        ...
        try:
            ...
            log('✓ Project verified in database:', GREEN)
