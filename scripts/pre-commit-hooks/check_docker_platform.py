...
def check_dockerfile_prod(filepath: Path) -> list[str]:
    ...
    # For production Dockerfiles, we should have --platform specified
    # or use a base image that's architecture-agnostic
    has_platform_spec = '--platform' in content

    if not has_platform_spec:
        # Check if there's a comment indicating this is handled elsewhere
...
def check_scripts_for_docker_build(filepath: Path) -> list[str]:
    ...
    for build_cmd in builds:
        ...
        if '--platform' not in build_cmd and 'buildx' not in build_cmd:
            # Check if it's building for production
...
def check_docker_compose_platform(filepath: Path) -> list[str]:
    ...
    # If the file references Dockerfile.prod, ensure platform is handled
...
def main() -> int:
    ...
    if errors:
        ...
        print('=' * 50)
        print()
        print('AWS ECS runs on linux/amd64 (x86_64).')
