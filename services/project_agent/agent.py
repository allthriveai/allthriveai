...
async def stream_agent_response(user_message: str, user_id: int, username: str, session_id: str):
    ...
    try:
        ...
        project_created = any(isinstance(m, ToolMessage) and 'project_id' in str(m.content) for m in messages)

        yield {'type': 'complete', 'session_id': session_id, 'project_created': project_created}
