"""
LLM-powered discovery agent using LangGraph.

Enables conversational project exploration with tools for:
- Searching projects
...
from .tools import DISCOVERY_TOOLS, TOOLS_BY_NAME, TOOLS_NEEDING_STATE
...
def get_llm_with_tools():
    """Get LLM instance with discovery tools bound (lazy initialization)."""
    global _llm_with_tools_instance
    if _llm_with_tools_instance is None:
        _llm_with_tools_instance = get_llm().bind_tools(DISCOVERY_TOOLS)
    return _llm_with_tools_instance


# Agent node
async def agent_node(state: DiscoveryAgentState) -> DiscoveryAgentState:
    """
    ...
    if not any(isinstance(m, SystemMessage) for m in messages):
        messages = [SystemMessage(content=DISCOVERY_SYSTEM_PROMPT)] + list(messages)
...
async def tool_node(state: DiscoveryAgentState) -> DiscoveryAgentState:
    """
    Execute tool calls from the last AI message.

    Manually injects state into tools that need user context.
    """
    logger.info('=== DISCOVERY TOOL_NODE CALLED ===')
    ...
    for tool_call in last_message.tool_calls:
        ...
        logger.info(f'Executing discovery tool: {tool_name} with args: {tool_args}')
        ...
        if not tool:
            ...
            try:
                ...
                if tool_name in TOOLS_NEEDING_STATE:
                    ...
                    logger.info(f'Injecting state with user_id={state.get("user_id")} into {tool_name}')
...
# Routing function
def should_continue(state: DiscoveryAgentState) -> str:
    """
...
def _build_workflow():
    """Build the discovery agent workflow graph."""
...
def create_discovery_agent():
    """Create stateless discovery agent for sync contexts."""
...
async def stream_discovery_response(user_message: str, user_id: int, username: str, session_id: str):
    """
    ...
    try:
        ...
        try:
            ...
            estimated_input_tokens = len(user_message) // 4 + len(DISCOVERY_SYSTEM_PROMPT) // 4
            ...
            AIUsageTracker.track_usage(
                user=user,
                feature='langgraph_discovery_agent',
            ...
            logger.warning(f'Failed to track LangGraph discovery usage: {tracking_error}')
        ...
        logger.error(f'Error in discovery agent stream: {e}', exc_info=True)
