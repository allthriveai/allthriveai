 ...
 * 2. "What is All Thrive" + Ember chat appears (right, fades in from logo cloud)
 * 3-5. Learn, Share, Play scenarios + Ember chat stays sticky
 */
...
type MessageType = 'user' | 'ember' | 'projects' | 'game' | 'cta' | 'url-input' | 'project-preview' | 'battle';
...
const allMessages: ConversationMessage[] = [
  ...
  { section: 'learn', type: 'ember', text: "The best way to learn is by playing! In Context Snake, you ARE the context window. Eat tokens → your context fills up. Grow too long → context overflow!" },
  { section: 'learn', type: 'game' },
  // Share section
  { section: 'share', type: 'user', text: "I just made my first chatbot! It's pretty basic but I'm proud of it." },
  { section: 'share', type: 'ember', text: "That's amazing! I'd love to help you share it. Just paste a URL or upload an image!" },
  { section: 'share', type: 'url-input' },
  { section: 'share', type: 'ember', text: "I made a project for you! Here's what it looks like:", showAvatar: true },
  { section: 'share', type: 'project-preview' },
  // See section
  { section: 'see', type: 'user', text: "Help me understand and choose a Vector Database" },
  { section: 'see', type: 'ember', text: "Great question! Here are some projects from the community that use vector databases:" },
  { section: 'see', type: 'projects' },
  // Connect section
  { section: 'connect', type: 'ember', text: "Prompt Battles are a fun way to practice your prompting skills and connect with the community!" },
  { section: 'connect', type: 'ember', text: "You both get the same challenge, write your best prompt, and AI judges the results. Ready to play?" },
];
...
// ============================================
// CHAT COMPONENTS (reused from MeetEmber)
// ============================================
...
function ChatBubble({ type, children, showAvatar = false }: { type: 'user' | 'ember'; children: React.ReactNode; showAvatar?: boolean }) {
  const isEmber = type === 'ember';
  return (
    <div className={`flex gap-3 ${isEmber ? '' : 'justify-end'}`}>
      {isEmber && showAvatar && (
        <div className="flex-shrink-0">
          <img src="/ember-avatar.png" alt="Ember" className="w-8 h-8 rounded-full border-2 border-cyan-500/50" />
        </div>
      )}
...
function EmberChatPanel({ activeSection, isGamePlaying, onStartGame }: { activeSection: SectionId; isGamePlaying: boolean; onStartGame: () => void }) {
      ...
      <div className="flex-1 overflow-y-auto p-5 scrollbar-hide">
        {activeSection === 'hero' ? (
          <div className="h-full flex items-center justify-center text-gray-500 text-sm">
            <p>Scroll down to start chatting with Ember</p>
          </div>
        ) : (
        ...
        <div className="relative z-10">
          ...
          <MobileFeatureSection
            ...
            chatContent={
              <div className="space-y-3">
                <MobileChatMessages
                  messages={getMessagesForSection('share').filter(m => m.type === 'user' || m.type === 'ember')}
                  isGamePlaying={false}
                  onStartGame={() => {}}
                />
                <div className="ml-8">
  ...
  return (
    ...
    <div ref={containerRef} className="relative" style={{ height: '500vh' }}>
      ...
      <div className="sticky top-0 h-screen overflow-hidden pt-16">
        ...
        {/* Right side - logo cloud / Ember chat */}
        <div className="w-[420px] xl:w-[480px] h-full p-6 pl-0 relative hidden lg:block">
          ...
          {/* Ember chat - fades in after hero, hidden on connect section */}
          {activeSection !== 'hero' && activeSection !== 'connect' && (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <EmberChatPanel activeSection={activeSection} isGamePlaying={isGamePlaying} onStartGame={() => setIsGamePlaying(true)} />
            </motion.div>
          )}
