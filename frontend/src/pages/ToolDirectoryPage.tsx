...
function ToolDirectoryPageContent() {
  ...
  // Tool request tray state
  const [isRequestTrayOpen, setIsRequestTrayOpen] = useState(false);
    ...
    <DashboardLayout>
      {() => (
        <div className="flex-1 overflow-y-auto h-full">
                          ...
                          {groupedTools[letter]?.map((tool) => {
                            ...
                            >
                              <div className="flex items-start gap-4">
                                <div className="flex-shrink-0 w-12 h-12 flex items-center justify-center overflow-hidden bg-white" style={{ borderRadius: 'var(--radius)' }}>
                                  {tool.logoUrl ? (
                                    <img
                                      src={tool.logoUrl}
                                      alt={`${tool.name} logo`}
          ...
          {/* Tool Request Tray */}
          <ToolRequestTray
            isOpen={isRequestTrayOpen}
            onClose={() => setIsRequestTrayOpen(false)}
          />
        </div>
      )}
    </DashboardLayout>
