...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    if (urlBetaCode && VALID_BETA_CODES.includes(urlBetaCode.toUpperCase())) {
      setBetaCode(urlBetaCode.toUpperCase());
      setIsBetaUnlocked(true);
    }
    // Also check localStorage for previously unlocked beta
    const storedBetaUnlocked = localStorage.getItem('betaUnlocked');
    if (storedBetaUnlocked === 'true') {
      setIsBetaUnlocked(true);
    }
  }, [searchParams]);

  const handleBetaCodeSubmit = () => {
    if (VALID_BETA_CODES.includes(betaCode.toUpperCase())) {
      setIsBetaUnlocked(true);
      setBetaCodeError('');
        ...
        >
          ...
          <motion.div variants={motionVariants} className="mb-8">
            ...
            <motion.p
              ...
              variants={motionVariants}
            >
              The gamified AI portfolio platform for anyone creating with AI. Automate your AI portfolio, compete in Prompt Battles, and level up your AI skills.
            </motion.p>
            <motion.p
          ...
          {/* Beta Code Gate or OAuth Buttons */}
          {!isBetaUnlocked ? (
            <motion.div
              className="mb-6"
              variants={motionVariants}
            >
              <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                ...
                <p className="text-sm text-slate-300 mb-4 text-center">
                  All Thrive is currently in private beta. Enter your access code to continue.
                </p>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent text-sm"
                    aria-label="Beta access code"
                  />
                  <motion.button
                    onClick={handleBetaCodeSubmit}
                    className="px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-green-500 text-white font-medium text-sm hover:from-cyan-400 hover:to-green-400 transition-all focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-background"
                ...
                {betaCodeError && (
                  <p className="mt-2 text-sm text-red-400 text-center">{betaCodeError}</p>
                )}
