...
export default function ProjectDetailPage() {
  ...
  const handleCopyLink = async () => {
    if (!project || !username || !projectSlug) return;

    const shareUrl = `${window.location.origin}/${username}/${projectSlug}`;
    try {
  ...
  const getShareUrls = () => {
    ...
    const shareUrl = encodeURIComponent(`${window.location.origin}/${username}/${projectSlug}`);
    const title = encodeURIComponent(project.title);
    const text = encodeURIComponent(`Check out "${project.title}" by @${project.username} on AllThrive AI`);
    const description = encodeURIComponent(project.description || `Check out ${project.title} by @${project.username}`);
