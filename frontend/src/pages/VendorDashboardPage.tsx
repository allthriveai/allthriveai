      ...
      <DashboardLayout>
        <div className="min-h-[80vh] flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            <div className="w-12 h-12 rounded-full border-2 border-cyan-500/30 border-t-cyan-bright animate-spin" />
        ...
        <div className="max-w-4xl mx-auto px-6 py-12">
          <div className="glass-card p-12 text-center">
            <div className="w-20 h-20 rounded-full bg-cyan-500/20 flex items-center justify-center mx-auto mb-6">
              <ChartBarIcon className="w-10 h-10 text-cyan-bright" />
            </div>
            <h2 className="text-2xl font-bold text-white mb-3">Welcome to Vendor Analytics</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-neon text-xs font-medium mb-3 tracking-wider uppercase">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="input-glass py-2 px-4 pr-10 text-sm min-w-[140px]"
              >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 border ${
                    selectedTool?.id === tool.id
                      ? 'bg-cyan-500/20 text-cyan-bright border-cyan-500/40 shadow-neon'
          ...
          <div className="space-y-8">
                ...
                <div className="flex items-start gap-6">
                  <div className="w-16 h-16 rounded-xl bg-white/10 flex items-center justify-center overflow-hidden flex-shrink-0">
                    {analytics.tool.logo_url ? (
                      ...
                      <SparklesIcon className="w-8 h-8 text-cyan-bright" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-2xl font-bold text-white">{analytics.tool.name}</h2>
                    ...
                    <p className="text-slate-400 mb-3">{analytics.tool.tagline}</p>
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-slate-500">Category: <span className="text-slate-300 capitalize">{analytics.tool.category}</span></span>
                      {metrics?.category_rank && (
                        <span className="text-slate-500">
                          Rank: <span className="text-cyan-bright font-medium">#{metrics.category_rank}</span>
              ...
              <div className="glass-card">
                ...
                <div className="flex items-center gap-8">
                  <div className="text-center">
                    <div className="text-4xl font-bold text-cyan-bright mb-1">#{metrics.category_rank}</div>
                    <div className="text-sm text-slate-400">Category Rank</div>
                  </div>
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-semibold text-white mb-1">{metrics.popularity_score}</div>
                    <div className="text-sm text-slate-400">Popularity Score</div>
                  </div>
                </div>
              </div>
            )}
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-300 font-medium">{review.user__username}</span>
                        </div>
            ...
            {analytics.similar_tools && analytics.similar_tools.length > 0 && (
              <div className="glass-card">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-lg font-semibold text-white">Similar Tools in Category</h3>
                    <p className="text-sm text-slate-400 mt-1">
                      Other tools in the {analytics.tool?.category} category
                    </p>
                  </div>
                  <ArrowTrendingUpIcon className="w-5 h-5 text-cyan-bright" />
                </div>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {analytics.similar_tools.map((tool) => (
                        <tr key={tool.id} className="hover:bg-white/5">
                          ...
                          <td className="py-3 text-right text-slate-300">{tool.view_count.toLocaleString()}</td>
                          <td className="py-3 text-right text-slate-300">{tool.popularity_score.toFixed(1)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
function MetricCard({
  ...
  const colorClasses = {
    cyan: 'bg-cyan-500/20 text-cyan-bright border-cyan-500/30',
  ...
  return (
    <div className="glass-card group hover:border-white/20 transition-all duration-300">
