"""Constants for GitLab repository analysis and import."""

# AI Analysis Limits (same as GitHub)
MAX_CATEGORIES_PER_PROJECT = 2
MAX_TOPICS_PER_PROJECT = 20
MAX_TOOLS_PER_PROJECT = 5
MIN_CATEGORY_ID = 1
MAX_CATEGORY_ID = 15

# Text Limits
MAX_DESCRIPTION_LENGTH = 500
MAX_TOPIC_LENGTH = 50

# API Limits
GITLAB_API_TIMEOUT = 10  # seconds
GITLAB_RETRY_ATTEMPTS = 3
GITLAB_RETRY_MIN_WAIT = 2  # seconds
GITLAB_RETRY_MAX_WAIT = 10  # seconds

# Import Rate Limits
IMPORT_RATE_LIMIT = '5/h'  # 5 imports per hour per user

# Import Lock and Task Timeouts
IMPORT_LOCK_TIMEOUT = 300  # 5 minutes - How long to hold the per-user import lock
IMPORT_TASK_SOFT_LIMIT = 300  # 5 minutes - Celery soft time limit (sends exception)
IMPORT_TASK_HARD_LIMIT = 360  # 6 minutes - Celery hard time limit (kills task)
