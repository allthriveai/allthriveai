...
class SocialConnection(models.Model):
    ...
    provider_username = models.CharField(
        max_length=255,
        blank=True,
        help_text='Username on the provider platform'
    )
    
    provider_email = models.EmailField(
        blank=True,
        null=True,
        help_text='Email from provider (if available)'
    )
    ...
    profile_url = models.URLField(
        blank=True,
        help_text='Link to user profile on provider'
    )
    
    avatar_url = models.URLField(
        blank=True,
        help_text='Profile picture from provider'
    )
