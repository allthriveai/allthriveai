...
class OAuthSetupTestCase(TestCase):
    ...
    def test_oauth_callback_url_format(self):
        ...
        self.assertEqual(github_callback, 'http://localhost:8000/accounts/github/login/callback/')
        self.assertEqual(google_callback, 'http://localhost:8000/accounts/google/login/callback/')
    ...
    def test_social_account_uid_is_unique_per_provider(self):
        ...
        with self.assertRaises(Exception):
            SocialAccount.objects.create(
                user=user2,
                provider='github',
                uid='12345'  # Duplicate UID for same provider
            )
